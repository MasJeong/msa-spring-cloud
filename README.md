...
**프로젝트 배경**
- 마이크로서비스 아키텍처 기술 스택(Spring Cloud, Kafka 등) 학습 및 실무 적용 가능성 탐구
- 확장 가능한 시스템 설계 역량 강화를 위한 자기개발 프로젝트
- 분산 시스템 운영 및 모니터링 경험 축적

**문제 정의**
- **확장성 한계**: 단일 애플리케이션은 특정 기능의 트래픽 증가 시 전체 시스템을 스케일링해야 하는 비효율성
- **강한 결합**: 모든 기능이 하나의 코드베이스에 묶여 있어 작은 변경도 전체 배포 및 장애 위험 증가
- **성능 및 안정성**: 한 부분의 병목이나 장애가 전체 시스템에 영향을 미치는 취약성
...
**비즈니스 가치**
- **확장성**: 수평 확장 가능한 아키텍처로 트래픽 증가 시 특정 서비스 인스턴스 추가만으로 대응 가능
...
**연결 관계:**
- **API Gateway** → 모든 서비스 routing, Redis Rate Limiting
- **User Service** → user_db
- **Order Service** → order_db + Kafka (Catalog 서비스 재고 차감 이벤트 발행)
- **Catalog Service** → catalog_db + Kafka (재고 업데이트 이벤트 구독)
- **Cart Service** → Redis (장바구니 데이터 저장)
- **File Service** → 독립 서비스 (WebDAV 사용)
...
- **실무 적용 장점**:
  ...
  - 트래픽 급증 시 이벤트 큐에 요청이 쌓여 Consumer가 처리 가능한 속도로 순차 처리되어 시스템 안정성 확보
  - Consumer 인스턴스의 처리 속도가 수신 속도를 따라가지 못하는 상황인 경우 Consumer 인스턴스만 추가하여 처리량 확보
...
- **구현 내용**: Eureka를 통한 자동 서비스 등록 및 Health Check
- **실무 적용 장점**:
...
- **구현 내용**: Git Private Repository 기반 설정 관리, Spring Cloud Bus를 통한 동적 갱신
- **실무 적용 장점**:
...
- **Jenkins Dashboard**: [AWS Jenkins Server](http://ec2-3-36-23-118.ap-northeast-2.compute.amazonaws.com:8080/login)

#### 자동화된 빌드 및 배포
- **설계 의도**: 수동 배포 과정 자동화로 배포 시간 단축
...
| **Discovery Service** | 8761 | 서비스 관리 서비스(Eureka) | 서비스 등록/발견, 헬스 체크 |
| **API Gateway** | 8000 | API 게이트웨이 서비스 | 라우팅, 인증, Rate Limiting, 로깅 |
| **User Service** | 랜덤 | 사용자 관리 서비스 | 회원가입, 로그인, 사용자 정보 관리 |
| **Order Service** | 랜덤 | 주문 관리 서비스 | 주문 생성, 주문 조회, Kafka 이벤트 발행 |
| **Catalog Service** | 랜덤 | 상품 카탈로그 서비스 | 상품 조회, 재고 관리, Kafka 이벤트 구독 |
| **Cart Service** | 랜덤 | 장바구니 서비스 | 장바구니 관리 및 Redis 캐싱 |
...
| **Prometheus** | 9090      | 메트릭 수집 | 시스템 메트릭 수집 |
| **Grafana** | 3001      | 모니터링 대시보드 | 메트릭 시각화 |
...
- [ ] 자동 스케일링 구현 (기회가 되는 경우)


## 📚 참고 자료

- [Spring Cloud 공식 문서](https://spring.io/projects/spring-cloud)
- [Resilience4J 문서](https://resilience4j.readme.io/v2.1.0/docs/circuitbreaker)
